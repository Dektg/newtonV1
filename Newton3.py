...
while True:
    ...
    if mess == ('стоп') or mess == ('топ'):
        exit(0)


    elif mess.find('создай') != -1 and mess.find('файл') != -1:
    	talk('как мне его назвать?')
    	x = myCommand()
    	talk('расширение файла?')
    	y = myCommand()
    	if y.find('питон') != -1 or y.find('Python') != -1:
    		os.system("touch " + x + ".py")
