...
while True:
    ...
    elif mess.find('произнеc') != -1 or mess.find('проговор') != -1:
        mess_split = mess.split(' ')
        clean_mess = []
        for a in mess_split:
            if a.find ('произнеc') != -1 and a.find('проговор') != -1:
                pass
            else:
                clean_mess.append(a)
        talk(' '.join(clean_mess))
...
# AZBUKA
    elif (mess.find('сдел') != -1 or mess.find('лучше') != -1 or mess.find('улучш') != -1 or mess.find('укрась') != -1)\
        and mess.find('текст') != -1 or mess.find('слов') != -1 or mess.find('красот') != -1 and mess.find('навед') != -1:
        if (mess.find('в ') != -1 or mess.find(' слов') != -1):
            try:
                clean_mess = []
                mess_split = mess.split(' ')
                for a in mess_split:
                    if a.find('сдел') != -1 or a.find('лучше') != -1 or a.find('улучш') != -1 or a.find('укрась') != -1 \
                        or a.find('текст') != -1 or a.find('красот') != -1 or a.find('навед') != -1 or a.find('в') != -1 or a.find('слов') != -1:
                        pass
                    else:
                        clean_mess.append(a)
                print(clean_mess)
                AZBUKA(' '.join(clean_mess))
            except:
                talk('Не могу, сэр')
        else:
            try:
                slovaa = input('Введите слово: ').lower()
                AZBUKA(slovaa)
            except:
                talk('Не могу, сэр')
...
# HOST_DNS
# HOST_IP
    elif (mess.find('найди') != -1 or mess.find('скажи')) != -1 and (mess.find(' хост') != -1
        or mess.find(' host') != -1 or mess.find(' админа') != -1 or mess.find(' admin') != -1):
        if mess.find(' админа') != -1 or mess.find(' admin') != -1:
            try:
        ...
        elif mess.find('днс') != -1 or mess.find('dns'):
            try:
                HOST_DNS()
            except:
                talk('Не могу, сэр')
